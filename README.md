...
- Richardson 외삽법 : 수치미분 정확도 개선
     ...
     미분값 그 자체를 기초로 하여 수치미분의 결과를 개선하는 방법들이 있다. 일반적으로 Richardson 외삽법이라고 하는 방법을 통해 두 개의 미분값을 적용하여 제3의 보다 정확한
     근사값을 계산한다.
     
     O(h^2)의 오차를 가지는 2개의 중심차분 공식에 의한 미분값을 조합하여 O(h^4)의 오차를 가지는 새로운 미분값을 계산하게 된다. 이때 구간간격이 절반이 되는(h2=h1/2)특수한 경우에
     다음과 같은 공식을 적용할 수 있다.
     
     <img src="https://latex.codecogs.com/png.image?\dpi{110}&space;\bg_white&space;D_{j,k}&space;=&space;\frac{4^{k-1}D_{j&plus;1,k-1}&space;-&space;D_{j,k-1}}{4^{k-1}-1}" title="\bg_white D_{j,k} = \frac{4^{k-1}D_{j+1,k-1} - D_{j,k-1}}{4^{k-1}-1}" />
     
     여기서 <img src="https://latex.codecogs.com/png.image?\dpi{110}&space;\bg_white&space;D_{j&plus;1,k-1},D_{j,k-1}" title="\bg_white D_{j+1,k-1},D_{j,k-1}" />은 각각 보다
     정확한 미분값과 덜 정확한 미분값이며 <img src="https://latex.codecogs.com/png.image?\dpi{110}&space;\bg_white&space;D_{j,k}" title="\bg_white D_{j,k}" />는 개선된 미분값이다.
     
     즉 k=1일 때, 원래의 중심차분 공식의 미분값에 해당하고 k=2일 때 O(h^4), k=3일 때 O(h^6)에 해당한다. 첨자j는 더정확하고 (j+1) 덜 정확한 추정값을 구별하기 위해 사용된다.
